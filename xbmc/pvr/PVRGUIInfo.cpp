...
void CPVRGUIInfo::Notify(Observable *obs, const CStdString& msg)
{
  if (obs == g_PVRTimers)
    UpdateTimersCache();
}
...
void CPVRGUIInfo::UpdateQualityData(void)
{
  ...
  CSingleLock lock(m_critSection);
  memcpy(&m_qualityInfo, &qualityInfo, sizeof(PVR_SIGNAL_STATUS));
}
...
void CPVRGUIInfo::UpdateBackendCache(void)
{
  ...
  if (iActiveClients > 0)
  {
    PVR_CLIENTMAP_ITR activeClient = activeClients.begin();
